...
class MyComBo_DonViNhan extends StatefulWidget {
  @override
  _MyComBo_DonViNhan createState() => new _MyComBo_DonViNhan();
}
...
List<int> lstDonVinhan = new List<int>();

class _MyComBo_DonViNhan extends State<MyComBo_DonViNhan> {
          ...
          if (snapshot.hasData) {
            ...
            if (list.length > 0) {
              ...
              return SearchableDropdown.multiple(
                items: lst,
                selectedItems: selectedItemsdvnhan,
                hint: Padding(
                  padding: const EdgeInsets.all(12.0),
                  child: Text("Chọn đơn vị nhận"),
                ),
                searchHint: "Chọn đơn vị nhận",
                onChanged: (value) {
                  lstDonVinhan = new List<int>();
                  setState(() {
                    strDonViDauMoi = '';
                    selectedItemsdvnhan = value;
                    selectedItemsdvnhan.forEach((element) {
                      strDonViDauMoi += ',' + lst[element].value.toString();
                      lstDonVinhan.add(int.parse(lst[element].value));
                    });
                  });
                },
